...
def get_gifs_from_giphy(search_string):
    ...
    api_search = 'api_key=' + api_key
    q = 'q=' + search_string
    limit = 'limit=5'
    others = 'offset=0&rating=G&lang=en'
    search_url = baseurl + api_search + '&' + q + '&' + limit + '&' + others
    r = requests.get(search_url)
    results_json = json.loads(r.text)
    return results_json['data']
...
def get_or_create_search_term(term):
    ...
    if search_term:
        ...
        search_results = get_gifs_from_giphy(term)
...
def index():
    ...
    if form.validate_on_submit():
        ...
        search_term = str(get_or_create_search_term(search_term).term)
